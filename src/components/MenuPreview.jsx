  ...
  return (
    <section id="menu-preview" className="py-20 px-4 bg-white">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Stats Row */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-16 reveal reveal-delay-2 px-4 sm:px-0">
          {[
            { num: "300+", label: lang === "en" ? "Pure Veg Dishes" : "శుద్ధ శాకాహారి వంటకాలు", icon: "🥘" },
            { num: "29",   label: lang === "en" ? "Menu Categories" : "మెనూ వర్గాలు", icon: "📑" },
            { num: "100%", label: lang === "en" ? "Quality Assured" : "నాణ్యత హామీ", icon: "✨" },
            { num: "33",   label: lang === "en" ? "Districts Served" : "సేవించిన జిల్లాలు", icon: "📍" },
          ].map((stat, idx) => (
            <div key={stat.num} className="bg-[#FCFBF8] border border-[#F2EFE9] rounded-2xl p-6 sm:p-8 flex flex-col items-center justify-center text-center transition-all duration-300 hover:-translate-y-2 hover:shadow-xl hover:border-saffron group">
               <div className="text-3xl mb-3 opacity-80 group-hover:scale-110 transition-transform duration-300">{stat.icon}</div>
               <div className="font-display text-4xl font-extrabold text-saffron mb-2">{stat.num}</div>
               <div className="font-body text-xs font-bold text-gray-500 uppercase tracking-widest">{stat.label}</div>
            </div>
          ))}
        </div>
